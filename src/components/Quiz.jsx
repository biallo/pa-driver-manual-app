...
export default function Quiz({ questions, version, scrollContainerRef, storageKey, text }) {
  ...
  }, [questions.length, scrollContainerRef, storageKey])

  useEffect(() => {
    const scrollTop = scrollContainerRef?.current?.scrollTop || 0
    saveStoredJson(storageKey, { selected, submitted, scrollTop })
  }, [selected, storageKey, submitted, scrollContainerRef])

  useEffect(() => {
    const scroller = scrollContainerRef?.current
    if (!scroller) return undefined
    const onScroll = () => {
      saveStoredJson(storageKey, { selected, submitted, scrollTop: scroller.scrollTop })
    }
  ...
  const clearProgress = () => {
    ...
    saveStoredJson(storageKey, { selected: {}, submitted: {}, scrollTop: 0 })
  }

  return (
    <div className="quiz-wrap">
      <div className="quiz-summary practice-summary">
        <div>{text.quiz.total(questions.length)}</div>
        <div>{text.quiz.score(score.correct)}</div>
        ...
        return (
          <div className="question-card" key={q.id}>
            ...
            <PageImage
              image={image}
              version={version}
              alt={text.quiz.questionImageAlt(questionNo)}
            />
            ...
            <div className="question-actions">
              {!hasAnswer && <span className="neutral">{text.quiz.noAnswer}</span>}
              {done && hasAnswer && (
                <span className={ok ? 'ok' : 'bad'}>
                  {ok ? text.quiz.correct : text.quiz.incorrect(q.correct)}
                </span>
              )}
            </div>
          </div>
